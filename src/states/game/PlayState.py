...
class PlayState(BaseState):
    ...
    def fixed_update(self) -> None:
        ...
        if self.flinging and self.can_split:
    ...
    def _update_idle(self) -> None:
        max_linear = self.bird.body.velocity.length()
        max_angular = abs(self.bird.body.angular_velocity)
        
        for b in self.extra_birds:
            max_linear = max(max_linear, b.body.velocity.length())
            max_angular = max(max_angular, abs(b.body.angular_velocity))

        if (
            max_linear < IDLE_LINEAR_SPEED_THRESHOLD
            and max_angular < IDLE_ANGULAR_SPEED_THRESHOLD
        ):
            self.idle_frames += 1
    ...
    def _split_bird(self) -> None:
        if self.flinging and not self.has_split and self.can_split:
            self.has_split = True
            v = self.bird.body.velocity
            speed = v.length()
            if speed > 0:
                angle = math.atan2(v.y, v.x)
                for offset_angle in (settings.SPLIT_ANGLE, -settings.SPLIT_ANGLE):
                    new_bird = Bird(self.world, self.bird.position.x, self.bird.position.y)
                    new_bird.body.angle = self.bird.body.angle
                    new_bird.body.angular_velocity = self.bird.body.angular_velocity
                    
                    new_v_angle = angle + offset_angle
                    new_v = pygame.Vector2(math.cos(new_v_angle) * speed, math.sin(new_v_angle) * speed)
                    new_bird.body.velocity = new_v
